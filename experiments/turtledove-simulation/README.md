...
We propose experiments to simulate ad personalization in the absence of third party cookies in the spirit of Chrome’s [TURTLEDOVE](https://github.com/WICG/turtledove) proposal. The central idea behind this proposal is to prevent cross-site tracking of users by separating advertiser user data from publisher contextual data, e.g., URLs, in ad requests. This is accomplished by splitting each ad request into two separate requests:

* A **contextual** request with user identifiers redacted
* An **interest group** request with only (anonymized) user identifiers but without any contextual signals. 
...
2. Designing and iterating on the RTB interface changes and impact on various aspects of RTB integration (such as publisher controls and advertiser/publisher brand safety, policies, creative scanning)
3. Experimenting with variables that aren't yet specified by the TURTLEDOVE proposal, or additional unknowns / features desirable by the ad tech industry.
